...
| 交易 | 年度 + 交易ID | 获取完整的交易数据 |
| 区块 | 区块号 or 年度 + 区块ID | 获取完整的区块数据 |
| 区块头链 | 创始块ID | 主要用于初始上线节点下载此基准数据 |
| UTXO 项 | 年度 + 交易ID + 输出序位 | 查询目标币金是否已花费 |
| UTCO 项 | 年度 + 交易ID + 输出序位 | 查询目标凭信是否已转移 |
| UTXO 集 | 创始块ID | 获取当前最新 UTXO 集 |
| UTCO 集 | 创始块ID | 获取当前最新 UTCO 集 |
| 交易ID组 | 年度 + 区块号 + 交易ID片段 | 用交易ID的前段局部模糊匹配其它交易ID（成组），仅返回完整交易ID组 |
| 附件ID | 年度 + 交易ID + 输出序位 | 获取目标脚本关联的附件信息 |
| 小附件 | 年度 + 交易ID + 输出序位 | 获取目标脚本关联的附件，仅限于 `<10MB` 的小附件 |

> **注：**
> Blockqs服务器与区块链节点连接，可以实时接收其广播的交易并暂存。
