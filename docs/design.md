...
```go
// 区块头链
2025/                               // 区块年度（按区块高度，87661区块/年计算）
    blocks.head                     // 区块头链数据，按恒星年（87661区块/年）计算
...
2025/                       // 区块年度
    001/                    // 日次：按240块/日计算
        [height].txl        // 区块：交易ID清单，按收录入区块相同顺序
        ...                 //
        chksum.list         // 各文件校验和，只读可选
    366/                    // 末尾余出的61个区块（87661 - (365 * 240) = 61）
        ...
```
